...
# DECAF_EXTERNAL_PROJECT_SETUP()
# Internal helper for DECAF_EXTERNAL_PROJECT.
AC_DEFUN([DECAF_EXTERNAL_PROJECT_SETUP], [
  ...
  AC_SUBST([DECAF_EXTERNAL_PROJECT_SUBDIRS], [""])
])

# DECAF_EXTERNAL_PROJECT(SUBDIR [, ADDITIONAL-CONFIGURE-ARGS])
# Setup SUBDIR as an external project. This means:
# - Execute the configure script immediately at the point of macro invocation.
# - Add SUBDIR to the substitution variable DECAF_EXTERNAL_PROJECT_SUBDIRS,
#   for the Makefile.in to arrange to execute make in the subdir.
#
# Derived from APR_SUBDIR_CONFIG
AC_DEFUN([DECAF_EXTERNAL_PROJECT], [
  AC_REQUIRE([DECAF_EXTERNAL_PROJECT_SETUP])
  DECAF_EXTERNAL_PROJECT_SUBDIRS="$DECAF_EXTERNAL_PROJECT_SUBDIRS $1"
